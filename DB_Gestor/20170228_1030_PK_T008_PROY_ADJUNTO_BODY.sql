create or replace PACKAGE BODY          "PK_T008_PROY_ADJUNTO" is
  ...
  PROCEDURE Pr_ConsultarPorFiltro (p_Filtro IN VARCHAR2,
    ...
    p_Temp1              VARCHAR2(200);
    p_Temp2              VARCHAR2(200);
    p_Temp3              VARCHAR2(200);
   ...
   BEGIN
      ...
      p_Temp1 := q'{ AND NVL(A008ESTADOREGISTRO,'}' || PK_UTL_CONSTANTE.COD_ACTIVO  || q'{') = '}'  || PK_UTL_CONSTANTE.COD_ACTIVO || q'{' }';
      p_Temp2 := q'{ AND NVL(A025ESTADOREGISTRO,'}' || PK_UTL_CONSTANTE.COD_ACTIVO  || q'{') = '}'  || PK_UTL_CONSTANTE.COD_ACTIVO || q'{' }';
      p_Temp3 := q'{ AND NVL(A026ESTADOREGISTRO,'}' || PK_UTL_CONSTANTE.COD_ACTIVO  || q'{') = '}'  || PK_UTL_CONSTANTE.COD_ACTIVO || q'{' }';
      strSQL := 'SELECT * FROM GPYT_T025_ADJUNTO AD LEFT JOIN   (    SELECT PA.*, AR.* FROM GPYT_T008_PROY_ADJUNTO PA     LEFT JOIN GPYT_T026_ARCHIVO AR ON A008IDARCHV = A026CODIGO     LEFT JOIN GPYT_T025_ADJUNTO AD ON A008IDADJNT = A025CODIGO     WHERE ' || p_Filtro  || p_Temp1 || p_Temp2 || p_Temp3 || '   ) REGISTRADOS  ON A008IDADJNT = A025CODIGO WHERE ' || p_Filtro  || p_Temp1 || p_Temp2 || p_Temp3 || 'ORDER BY A025CODIGO';

      PK_UTL_UTILIDAD.Pr_RegistrarDebug (p_niveldebug    => PK_UTL_CONSTANTE.NIVEL_TRACE,
                                         p_paquete       => NOMBRE_PAQUETE,
                                         p_procedimiento => NOMBRE_PROCEDIMIENTO,
                                         p_usuario       => p_idUsuario,
                                        p_descripcion   => 'strSQL ' || strSQL);
